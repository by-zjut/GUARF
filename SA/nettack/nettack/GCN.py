    ...
    def __init__(self, sizes, An, X_obs, name="", with_relu=True, params_dict={'dropout': 0.5}, gpu_id=0,
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope(name) as scope:
                ...
                self.W1 = tf.Variable(w_init(shape=[self.D, sizes[0]]), dtype=tf.float32, name='W1')
                # self.b1 = slim.variable('b1', dtype=tf.float32, initializer=tf.zeros(sizes[0]))
                self.b1 = tf.Variable(tf.zeros(shape=[sizes[0]]), dtype=tf.float32, name='b1')
                ...
                self.W2 = tf.Variable(w_init(shape=[sizes[0], sizes[1]]), dtype=tf.float32, name='W2')
                self.b2 = tf.Variable(tf.zeros(shape=[sizes[1]]), dtype=tf.float32, name='b2')
